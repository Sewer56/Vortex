 ...
 * hand-mirror of `src/renderer/src/util/installerHelpers.ts`. We can't import
 * the canonical implementation because it transitively pulls in `getGame`,
 * which requires the renderer's redux-singleton initialisation. The drift
 * guard lives at `packages/game-extension-test/src/declareInstallers.test.ts`
 * — keep it green when changing this file or the canonical helpers.
 */
